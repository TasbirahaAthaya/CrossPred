...
library(limma)
rdata <- read.csv('./BC_allcodes/bc_exnonc.csv',row.names='ID')
exdata <- data.frame(rdata)
# print(exdata['nc_sub_115'])

rdata1 <- read.csv('./BC_allcodes/bc_exdata_design.csv',row.names='index')
...
# Write CSV
write.csv(top_genes, "./BC_allcodes/bc_ex2550.csv")
