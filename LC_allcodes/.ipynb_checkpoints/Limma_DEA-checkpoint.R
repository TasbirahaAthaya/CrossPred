library(limma)
rdata <- read.csv('C:/UCF/exRNAProject/Codes&Data/lc_exnonc.csv',row.names='ID')
exdata <- data.frame(rdata)
print(exdata['nc_sub_115'])

rdata1 <- read.csv('C:/UCF/exRNAProject/Codes&Data/lc_exdata_design.csv',row.names='index')
...
# Write CSV
write.csv(top_genes, "lc_ex2550.csv")